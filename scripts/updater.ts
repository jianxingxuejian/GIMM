...
export async function updater() {
  ...
  const downloadUrl = 'https://github.com/jianxingxuejian/GIMM/releases/download/' + tag_name + '/'
  const version = tag_name.slice(1)

  const downloadUrl_win = downloadUrl + `GIMM_${version}_x64_en-US.msi.zip`
  const signature_win = await getSignature(downloadUrl_win)
  const downloadUrl_darwin = downloadUrl + 'GIMM.app.tar.gz'
  const signature_darwin = await getSignature(downloadUrl_darwin)
  const download_linux = downloadUrl + `GIMM_${version}_amd64.AppImage.tar.gz`
